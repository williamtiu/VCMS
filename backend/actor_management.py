...
DEFAULT_DB_PATH_MODULE: str = os.path.join(
    _PROJECT_ROOT, "database", "video_management.db"
)
...
def add_actor(db_path: str, actor_name: str) -> Optional[int]:
    ...
    Returns:
        Optional[int]: The ID of the newly added or existing actor, or None
                       if an error occurs or actor_name is empty.
    """
    ...
    try:
        with _get_db_connection(db_path) as conn:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id FROM actors WHERE name = ?", (normalized_name_for_check,)
            )
            row = cursor.fetchone()
            if row:
                logger.info(
                    f"Actor '{normalized_name_for_check}' already exists with ID: {row['id']}."
                )
                return row["id"]

            cursor.execute(
                "INSERT INTO actors (name) VALUES (?)", (normalized_name_for_check,)
            )
            new_actor_id = cursor.lastrowid
            logger.info(
                f"Actor '{normalized_name_for_check}' added with ID: {new_actor_id}."
            )
            return new_actor_id
    except sqlite3.IntegrityError as e:
        logger.warning(
            f"Integrity error adding actor '{normalized_name_for_check}': {e}. Attempting to retrieve existing ID.",
            exc_info=True,
        )
        try:
            # Retry connection for safety, though existing conn might be usable if not rolled back by context manager on specific errors
            with _get_db_connection(db_path) as conn_retry:
                ...
                if row_retry:
                    logger.info(
                        f"Found existing actor '{normalized_name_for_check}' with ID {row_retry['id']} after integrity error."
                    )
                    return row_retry["id"]
        except sqlite3.Error as e_retry:
            logger.error(
                f"Error during retry-fetch for actor '{normalized_name_for_check}': {e_retry}",
                exc_info=True,
            )
        return None
    except sqlite3.Error as e:
        logger.error(
            f"Database error while adding actor '{normalized_name_for_check}': {e}",
            exc_info=True,
        )
        return None
...
def add_alias(db_path: str, actor_id: int, alias_name: str) -> bool:
    ...
    if not alias_name or not alias_name.strip():
        ...
        return False
    if actor_id is None: # Should be caught by type hint, but good runtime check
    ...
    try:
        with _get_db_connection(db_path) as conn:
            ...
            if not cursor.fetchone():
                logger.warning(
                    f"Attempted to add alias for non-existent actor ID: {actor_id}."
                )
                return False
            cursor.execute(
            ...
            if row:
                if row["actor_id"] == actor_id:
                    logger.info(
                        f"Alias '{normalized_alias_name}' already exists for actor ID {actor_id}."
                    )
                else:
                    logger.warning(
                        f"Alias '{normalized_alias_name}' already exists for a different actor (ID: {row['actor_id']}). Cannot add."
                    )
                return False  # Alias exists, so not added "successfully" in terms of a new addition

            cursor.execute(
                "INSERT INTO actor_aliases (actor_id, alias_name) VALUES (?, ?)",
                (actor_id, normalized_alias_name),
            )
            logger.info(
                f"Alias '{normalized_alias_name}' added for actor ID {actor_id}."
            )
            return True
    except sqlite3.IntegrityError as e:
        logger.warning(
            f"Integrity error adding alias '{normalized_alias_name}' for actor ID {actor_id}: {e}.",
            exc_info=True,
        )
        return False
    except sqlite3.Error as e:
        logger.error(
            f"Database error while adding alias '{normalized_alias_name}' for actor ID {actor_id}: {e}",
            exc_info=True,
        )
        return False


def get_actor_id_by_name_or_alias(
    ...
    try:
        with _get_db_connection(db_path) as conn:
            ...
            if row:
                logger.debug(
                    f"Found actor ID {row['id']} by direct name match for '{normalized_name}'."
                )
                return row["id"]
            cursor.execute(
                "SELECT actor_id FROM actor_aliases WHERE alias_name = ?",
            ...
            if row:
                logger.debug(
                    f"Found actor ID {row['actor_id']} by alias match for '{normalized_name}'."
                )
                return row["actor_id"]
        ...
        logger.error(
            f"Database error while searching for name/alias '{normalized_name}': {e}",
            exc_info=True,
        )
        return None
...
def get_aliases_for_actor(db_path: str, actor_id: int) -> List[str]:
    ...
    try:
        ...
        logger.error(
            f"Database error while fetching aliases for actor ID {actor_id}: {e}",
            exc_info=True,
        )
        return []
...
def get_actor_name_by_id(db_path: str, actor_id: int) -> Optional[str]:
    ...
    try:
        ...
        logger.error(
            f"Database error while fetching name for actor ID {actor_id}: {e}",
            exc_info=True,
        )
        return None


def get_all_actors_with_aliases(
    db_path: str,
    ...
    try:
        ...
        logger.error(
            f"Unexpected error in get_all_actors_with_aliases: {e}", exc_info=True
        )
        return []
...
if __name__ == "__main__":
    ...
    TEST_ACTOR_DB_PATH: str = os.path.join(
        _PROJECT_ROOT, "database", "actor_management_test.db"
    )

    logger.info(
        f"--- Testing actor_management.py with dedicated test DB: {TEST_ACTOR_DB_PATH} ---"
    )
    ...
    try:
        ...
        if test_conn:
            ...
            logger.critical(
                f"Failed to connect to test DB {TEST_ACTOR_DB_PATH} for setup. Aborting."
            )
            sys.exit(1)

    except ImportError as ie:
        logger.error(
            f"Could not import from database_setup.py: {ie}. Schema might be missing for tests.",
            exc_info=True,
        )
        sys.exit(1)
    except Exception as e:
        logger.error(
            f"Error setting up test DB using database_setup functions: {e}",
            exc_info=True,
        )
        sys.exit(1)
    ...
    logger.info(
        f"Test 2 Passed: Adding existing 'Test Actor Main' returned ID {existing_id}"
    )

    success_alias = add_alias(TEST_ACTOR_DB_PATH, actor1_id, "TAM") # type: ignore
    assert success_alias, "Test 3 Failed: Add new alias"
    logger.info("Test 3 Passed: Added alias 'TAM'")

    fail_alias_exists = add_alias(TEST_ACTOR_DB_PATH, actor1_id, "TAM") # type: ignore
    assert not fail_alias_exists, "Test 4 Failed: Add existing alias for same actor"
    logger.info(
        "Test 4 Passed: Attempt to re-add 'TAM' for same actor failed as expected."
    )

    actor_john_doe_id = 1  # from sample data
    ...
    logger.info(
        "Test 5 Passed: Attempt to add globally existing alias 'TAM' to John Doe failed."
    )
    ...
    logger.info(
        "Test 6 Passed: Attempt to add alias for non-existent actor ID 9999 failed."
    )

    found_id_main = get_actor_id_by_name_or_alias(
        TEST_ACTOR_DB_PATH, "Test Actor Main"
    )
    ...
    found_id_none = get_actor_id_by_name_or_alias(
        TEST_ACTOR_DB_PATH, "NonExistent Actor"
    )
    ...
    aliases_for_a1 = get_aliases_for_actor(TEST_ACTOR_DB_PATH, actor1_id) # type: ignore
    assert sorted(aliases_for_a1) == sorted(
        ["TAM"]
    ), f"Test 10 Failed: Get aliases. Expected ['TAM'], got {aliases_for_a1}"
    ...
    assert sorted(aliases_for_jd) == sorted(
        ["J. Doe", "Johnny D"]
    ), f"Test 11 Failed: Get John Doe aliases. Expected ['J. Doe', 'Johnny D'], got {aliases_for_jd}"
    logger.info(
        f"Test 11 Passed: Aliases for John Doe (ID {actor_john_doe_id}): {aliases_for_jd}"
    )

    name_for_a1 = get_actor_name_by_id(TEST_ACTOR_DB_PATH, actor1_id) # type: ignore
    assert (
        name_for_a1 == "Test Actor Main"
    ), f"Test 12 Failed: Get name by ID. Expected 'Test Actor Main', got {name_for_a1}"
    ...
    logger.info(
        "Test 13 Passed: Non-existent actor ID 9999 correctly returned None for name."
    )
    ...
    assert (
        len(all_actors) >= 3
    ), f"Test 14 Failed: Expected at least 3 actors, got {len(all_actors)}"
    logger.info(f"Test 14 Passed: Retrieved {len(all_actors)} actors.")
    for actor_data in all_actors:
        logger.debug(
            f"  Actor: ID={actor_data['id']}, Name='{actor_data['name']}', Aliases={actor_data['aliases']}"
        )
        if actor_data["id"] == actor1_id:
    ...
    logger.info(
        "Verified specific actor data in get_all_actors_with_aliases output."
    )
