...
# and ensures that when imported, 'backend' is correctly identified as a package if the
# project root is in sys.path.
SCRIPT_DIR_FOR_PATH_MOD = os.path.dirname(os.path.abspath(__file__)) # backend directory
PROJECT_ROOT_FOR_PATH_MOD = os.path.dirname(SCRIPT_DIR_FOR_PATH_MOD) # Project root (/app)
...
from backend.actor_management import get_actor_id_by_name_or_alias, get_actor_name_by_id, add_actor as backend_add_actor # Correctly import and alias
from ai_models.content_analysis import enhance_textual_metadata # analyze_transcribed_audio (if used later)
from ai_models.llm_analyzer import configure_ollama_client # For explicit configuration
...
DEFAULT_DB_PATH_MODULE: str = os.path.join(_PROJECT_ROOT_MODULE, 'database', 'video_management.db')
...
def generate_standardized_filename(consolidated_metadata: Dict[str, Any], original_extension: str) -> str:
    """
    Constructs a standardized filename string based on consolidated metadata.
    Pattern: "[Code/Publisher] Title - Actor1, Actor2.extension" (parts omitted if not available).
    ...
    actors = consolidated_metadata.get('actors', []) # List of {'canonical_name': ...}

    # 1. Code or Publisher part
    code_publisher_str = ""
    if code:
    ...
    if title:
        ...
        filename_parts.append("Unknown_Title") # Ensure a title is always present

    # 3. Actors part
    if actors:
        # Ensure actors is a list of dicts and 'canonical_name' exists
        actor_names = sorted([
    ...
    # Replace multiple spaces that might result from joining empty/whitespace-only parts
    base_name = re.sub(r'\s{2,}', ' ', base_name).strip()


    if not base_name or base_name == "Unknown_Title" and not code_publisher_str and not actors:
        # Fallback if all components were missing or resulted in an empty string
        # (e.g. title was None and no code/pub/actors)
        # Use original filename without extension, sanitized.
        original_base = os.path.splitext(consolidated_metadata.get("original_filename_for_fallback", "Untitled_Video"))[0]
    ...
    if len(base_name) > max_len:
        base_name = base_name[:max_len].strip(" _-.") # Also strip common trailing separators

    valid_extension = original_extension if original_extension and original_extension.startswith('.') else '.unknown'
    return f"{base_name}{valid_extension}"

def _prepare_text_for_ai(parsed_info: Dict[str, Any],
                         current_actors_map: Dict[Union[int, str], Dict[str, Any]],
                         original_filename_with_ext: str) -> str:
    """
    ...
    Args:
        ...
        current_actors_map (Dict[Union[int, str], Dict[str, Any]]): Map of actors derived from filename.
    ...
    # Get actor names from the current_actors_map that came from filename parsing
    filename_actor_names = [
        data['canonical_name'] for data in current_actors_map.values()
        if data.get('source', '').startswith('filename') and data.get('canonical_name')
    ]
    ...
    if not text_for_ai:
        text_for_ai = original_filename_with_ext # Fallback to the full original filename

    # Basic cleaning
    text_for_ai = text_for_ai.replace('_', ' ').replace('.', ' ').replace('-', ' ')
    text_for_ai = re.sub(r'\s+', ' ', text_for_ai).strip() # Consolidate multiple spaces

    logger.debug(f"Prepared text for AI analysis: '{text_for_ai}'")
    return text_for_ai

def _consolidate_title(parsed_title: Optional[str],
                       llm_suggested_title: Optional[str],
                       fallback_filename_part: str) -> str:
    """
    ...
    Args:
        ...
        fallback_filename_part (str): A fallback string, usually cleaned original filename base.
    ...
    if llm_suggested_title:
        # Prefer LLM title if original is missing, very short, generic, or just numbers
        if not final_title or len(final_title) < 5 or \
           "untitled" in final_title.lower() or \
           final_title.isdigit() or \
           final_title.lower() == os.path.splitext(fallback_filename_part)[0].lower().replace('_', ' ').replace('.', ' '): # if parsed title is just filename
            final_title = llm_suggested_title
            logger.info(f"Using LLM suggested title: '{final_title}'")
        elif parsed_title: # Parsed title exists and seems okay, log that we kept it
            logger.info(f"Keeping filename-parsed title '{parsed_title}' over LLM suggestion '{llm_suggested_title}'.")

    if not final_title: # Fallback if still no title
         final_title = _sanitize_filename_part(os.path.splitext(fallback_filename_part)[0])
         if not final_title: final_title = "Unknown Title" # Absolute fallback
...
def _consolidate_actors(
    ...
    Args:
        current_actors_map (Dict[Union[int, str], Dict[str, Any]]):
            Existing map of actors, potentially from filename parsing.
            Keys are actor_id (if known) or lowercase actor_name.
    ...
    if llm_identified_actors:
        logger.debug(f"Consolidating LLM actors: {llm_identified_actors} with existing map: {current_actors_map}")
        for actor_name_llm in llm_identified_actors:
            actor_id = get_actor_id_by_name_or_alias(db_path, actor_name_llm)
            canonical_name = actor_name_llm # Default to name from LLM
            source_tag = 'llm_no_db_match'

            if actor_id: # Actor found in DB
                name_from_db = get_actor_name_by_id(db_path, actor_id)
                if name_from_db: canonical_name = name_from_db
                source_tag = 'llm_db_match'
                # If this ID is already in map, update if new source is more "verified" (e.g. llm_db_match vs filename_no_db_match)
                # For now, just ensure it's there with the DB canonical name.
                if actor_id not in current_actors_map:
                    current_actors_map[actor_id] = {'id': actor_id, 'canonical_name': canonical_name, 'source': source_tag}
                else: # Actor ID already exists, ensure canonical name is from DB if possible
                    current_actors_map[actor_id]['canonical_name'] = canonical_name
            else: # LLM found an actor not in DB
                # Optionally, auto-add this actor to the DB here if desired:
                # new_actor_id = backend_add_actor(db_path, actor_name_llm)
                # if new_actor_id:
                #   current_actors_map[new_actor_id] = {'id': new_actor_id, 'canonical_name': actor_name_llm, 'source': 'llm_added_to_db'}
                # else: # Failed to add, store by name
                #   if actor_name_llm.lower() not in current_actors_map:
                #       current_actors_map[actor_name_llm.lower()] = {'id': None, 'canonical_name': actor_name_llm, 'source': source_tag}

                # For now, just record without auto-adding to DB from here:
                if actor_name_llm.lower() not in current_actors_map: # Check against existing non-DB actors by name
                    current_actors_map[actor_name_llm.lower()] = {'id': None, 'canonical_name': actor_name_llm, 'source': source_tag}

    # Prioritize actors with IDs, then those without, maintaining some order from map.
    # Note: dicts are ordered from Python 3.7+
    final_list = [data for data in current_actors_map.values()]
    return final_list

def _consolidate_publisher(parsed_publisher: Optional[str], llm_identified_publisher: Optional[str]) -> Optional[str]:
    """
    ...
    if llm_identified_publisher:
        logger.info(f"Using LLM identified publisher: '{llm_identified_publisher}'")
        return llm_identified_publisher
    if parsed_publisher: # Fallback to parsed, though current parser doesn't extract it
        logger.info(f"Using filename-parsed publisher: '{parsed_publisher}' (rare)")
        return parsed_publisher
    return None


def process_video_file(video_filepath: str, db_path: str = DEFAULT_DB_PATH_MODULE) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        # 2. Initial Actor Processing from Filename
        ...
        for actor_name_fn in parsed_info.get("actors", []):
            ...
            if actor_id:
                ...
                if db_name: canonical_name = db_name
        ...
        run_content_analysis: bool = not (parsed_info.get("title") and \
                                        (parsed_info.get("actors", []) or any(val['id'] for val in actors_map.values())) and \
                                        parsed_info.get("code"))
        if run_content_analysis:
            logger.info("Flagged for AI content analysis due to missing/incomplete filename metadata.")

        # 4. AI Content Analysis
        ai_enhancements: Optional[Dict[str, Any]] = None
        if run_content_analysis:
            text_for_ai = _prepare_text_for_ai(parsed_info, actors_map, original_filename_with_ext)
            ai_enhancements = enhance_textual_metadata(text_input=text_for_ai, original_filename=original_filename_with_ext)

        raw_ai_results = {"text_enhancements": ai_enhancements, "audio_analysis": None} # Assuming no audio analysis for now

        # 5. Consolidate Metadata
        llm_title = ai_enhancements.get("llm_suggested_title") if ai_enhancements else None
        final_title = _consolidate_title(parsed_info.get("title"), llm_title, original_filename_with_ext)

        llm_actors = ai_enhancements.get("llm_identified_actors") if ai_enhancements else None
        final_actors_list = _consolidate_actors(actors_map, llm_actors, db_path) # actors_map is updated in place

        llm_publisher = ai_enhancements.get("llm_identified_publisher") if ai_enhancements else None
        final_publisher = _consolidate_publisher(None, llm_publisher) # Assuming parser doesn't get publisher
        ...
        consolidated_metadata: Dict[str, Any] = {
            ...
            "original_filename_for_fallback": original_filename_with_ext, # For generate_standardized_filename
            ...
            'ai_network_actor_info': ai_enhancements.get('network_search_actor_results', []) if ai_enhancements else [],
            'ai_network_publisher_info': ai_enhancements.get('network_search_publisher_results', []) if ai_enhancements else [],
        }

        actors_for_db_and_filename = [actor for actor in final_actors_list if actor.get("id") is not None]
        # Use a temporary dict for filename generation that only has actors with IDs for consistency
        ...
        logger.info(f"Generated standardized filename: {consolidated_metadata['standardized_filename']}")

        # 6. Database Update
        update_video_record(
            db_path,
            consolidated_metadata["filepath"], consolidated_metadata["code"],
            consolidated_metadata["title"], consolidated_metadata["publisher"],
            consolidated_metadata["duration_seconds"], consolidated_metadata["standardized_filename"],
        ...
        logger.error(f"Unexpected error in process_video_file for '{video_filepath}': {e}", exc_info=True)
        return None
...
if __name__ == '__main__':
    ...
    # Imports specific to __main__ if they were not at top level (e.g. for setup)
    from database.database_setup import create_connection as setup_create_conn, create_all_tables, insert_sample_data
    ...
    configure_ollama_client() # Attempt to configure the global client from llm_analyzer

    # Use a dedicated test DB, path constructed using the globally defined PROJECT_ROOT_FOR_PATH_MOD
    test_db_path = os.path.join(PROJECT_ROOT_FOR_PATH_MOD, 'database', 'metadata_processor_test.db')

    if os.path.exists(test_db_path):
        logger.info(f"Deleting existing test database: {test_db_path}")
        os.remove(test_db_path)

    # Setup test DB schema and sample data
    conn_setup = setup_create_conn(test_db_path) # Uses function from database_setup
    if conn_setup:
        try:
            logger.info(f"Creating tables in {test_db_path}...")
            if not create_all_tables(conn_setup): # Uses function from database_setup
                logger.critical("Failed to create tables for test DB. Aborting.")
                sys.exit(1)
            logger.info(f"Inserting sample data into {test_db_path}...")
            if not insert_sample_data(conn_setup): # Uses function from database_setup
        ...
        logger.critical(f"Failed to connect to test DB {test_db_path} for setup. Aborting.")
        sys.exit(1)

    videos_dir = os.path.join(PROJECT_ROOT_FOR_PATH_MOD, "data", "videos_metadata_test_ai_refactored")
    os.makedirs(videos_dir, exist_ok=True)

    dummy_files_info = [
        {"name": "[XYZ-789] My Great Movie - John Doe.mp4", "content": "movie dummy"},
        {"name": "raw_clip_001.avi", "content": "raw dummy"},
        {"name": "CookingShow_Ep03_Delicious_Cakes.mkv", "content": "cooking dummy"},
        {"name": "Tech Review The New Phone X.mp4", "content": "tech dummy"},
        {"name": "Performance_UnknownArtist.webm", "content":"performance dummy"}
    ]
    ...
    for video_path in test_video_paths:
        ...
        if result:
            logger.info(f"Full processing result for {video_path}:\n{json.dumps(result, indent=4)}")
        else:
    ...
    try:
        ...
        logger.error(f"Error during database verification: {e_verify}", exc_info=True)
    finally:
        ...
        if os.path.exists(videos_dir):
            try:
                ...
                logger.info(f"Cleaned up test directory: {videos_dir}")
                 ...
                 logger.warning(f"Error removing test directory {videos_dir}: {e_cleanup_dir}")
