...
from typing import List, Dict, Optional, Any # Added Any for DDGS result dict
...
def search_web(query: str, max_results: int = DEFAULT_MAX_RESULTS) -> Optional[List[Dict[str, str]]]:
    """
    ...
    try:
        ...
        return results # Returns empty list if no results, or list of results
...
def fetch_url_content(url: str, timeout: int = 10) -> Optional[str]:
    ...
    Removes script and style tags. Normalizes whitespace.
    ...
    try:
        ...
        headers: Dict[str, str] = {
            ...
            # Some sites might block default requests User-Agent or require others.
        }
        ...
        if not content_type or 'html' not in content_type:
            logger.warning(f"Content at {url} is not HTML (type: {content_type if content_type else 'Unknown'}). Skipping text extraction.")
            return None

        soup = BeautifulSoup(response.content, 'html.parser')

        # Remove script and style elements
        for script_or_style in soup(["script", "style", "header", "footer", "nav", "aside"]): # Added more common non-content tags
            script_or_style.decompose()

        # Get text, normalize whitespace
        text: str = soup.get_text(separator='\n', strip=True)

        # Further processing to reduce excessive newlines and spaces, then join with single spaces
        lines: List[str] = [line.strip() for line in text.splitlines() if line.strip()]
        # The previous logic for chunks might be too aggressive or complex.
        # Joining lines with space is usually sufficient after stripping.
        # If more specific phrase splitting is needed, it can be re-evaluated.
        # For now, a simpler join of processed lines:
        text = ' '.join(lines)

        if not text:
            logger.warning(f"No text content extracted from {url} after parsing.")
            return None # Return None if text is empty after processing

        logger.info(f"Successfully fetched and parsed content from {url} (extracted text length: {len(text)}).")
        return text
        ...
        return None
    except requests.exceptions.RequestException as e: # Other requests errors (timeout, connection error)
        logger.error(f"Request error fetching URL {url}: {e}", exc_info=True)
        return None
    except Exception as e: # Catch-all for other errors (e.g., BeautifulSoup parsing)
...
if __name__ == '__main__':
    ...
    logging.basicConfig(level=logging.DEBUG,
                        ...
                        force=True) # force=True to override any root logger config
    ...
    results1: Optional[List[Dict[str, str]]] = search_web(search_query1, max_results=3)
    if results1 is not None: # Check for None (search error) vs empty list (no results)
        if results1: # Check if list is not empty
    ...
    results2: Optional[List[Dict[str, str]]] = search_web(search_query2, max_results=2)
    if results2 is not None:
        if results2:
            for i, res in enumerate(results2):
                logger.info(f"  Result {i+1}: {res.get('title')} ({res.get('href')})")
        else:
    ...
    if results1 and results1[0] and results1[0].get('href'): # Check results1, then first item, then href
        first_url_to_fetch = results1[0]['href']
        logger.info(f"  Attempting to fetch content from first search result: {first_url_to_fetch}")
        content1: Optional[str] = fetch_url_content(first_url_to_fetch)
        if content1:
            logger.info(f"  Successfully fetched content (first 500 chars):\n{content1[:500]}...")
        else:
            logger.warning(f"  Failed to fetch content from {first_url_to_fetch}.")
    else:
        logger.info(f"  Skipping dynamic URL fetch for Test 3 as no prior search results were available or valid.")

    logger.info("\n--- Test 4: Fetch URL Content (guaranteed non-HTML) ---")
    url_non_html: str = "https://raw.githubusercontent.com/googleapis/google-api-python-client/main/README.md" # Raw MD
    content_non_html: Optional[str] = fetch_url_content(url_non_html)
    if content_non_html is None:
        logger.info(f"  Test 4 Passed: Correctly handled non-HTML URL: {url_non_html} (returned None).")
    else:
        logger.error(f"  Test 4 FAILED: Unexpectedly got content from non-HTML URL: {url_non_html}. Snippet: {str(content_non_html)[:100]}...")

    logger.info("\n--- Test 5: Fetch URL Content (non-existent URL) ---")
    url_bad: str = "http://thisdomainshouldreallynotexist123456789abcdef0.com/nonexistentpage.html" # Made more unique
    content_bad: Optional[str] = fetch_url_content(url_bad)
    if content_bad is None:
        logger.info(f"  Test 5 Passed: Correctly handled non-existent URL: {url_bad} (returned None).")
    else:
        logger.error(f"  Test 5 FAILED: Unexpectedly got content from non-existent URL: {url_bad}.")
